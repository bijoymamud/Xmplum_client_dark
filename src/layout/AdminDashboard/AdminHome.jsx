...
//           </div>
//           <h1 className="text-2xl font-bold text-gray-700 dark:text-[#D0CDEF]">10</h1>
//         </div>

//         <div className="dark:bg-[#1B1744] border border-white/20 shadow-md rounded-lg p-6">
//           <div className="flex items-center justify-between mb-4">
//             <h1 className="text-xl font-semibold text-gray-800 dark:text-[#D0CDEF]">Subscribers</h1>
//             <div className="p-3 bg-green-100 rounded-full text-green-600"><TbUserDollar className="text-2xl" /></div>
//           </div>
//           <h1 className="text-2xl font-bold text-gray-700 dark:text-[#D0CDEF]">5,132</h1>
//         </div>
...
//           </div>
//           <h1 className="text-2xl font-bold text-gray-700 dark:text-[#D0CDEF]">4,890</h1>
//         </div>
//       </div>

//       {/* PDF Upload Section */}
//       <div className="mt-6 dark:bg-[#1B1744] border border-white/20 shadow-md rounded-lg p-6">
...
//        className="flex items-center gap-2 dark:text-[#D0CDEF] font-semibold py-2 px-8 hover:dark:bg-[#222036] border border-white/30 shadow-sm shadow-amber-200">Upload PDF <IoCloudUploadOutline size={25}/>
...
const AdminHome = () => {
  ...
  const [uploadedFiles, setUploadedFiles] = useState([]);
  const [pdfPreviews, setPdfPreviews] = useState([]);
  const [filesToUpload, setFilesToUpload] = useState([]); // Store raw files for upload
  ...
  const onDrop = async (acceptedFiles) => {
    ...
    setFilesToUpload(pdfs); // Store the raw files for upload
    setUploadedFiles(pdfs.map(file => ({ name: file.name }))); // For display
  ...
  const handleUpload = async () => {
    ...
    // Append each file to FormData (backend expects files in FormData)
    filesToUpload.forEach((file) => {
      formData.append(`pdfs`, file); // Adjust key as per backend requirement
    });
  ...
  <div className="mt-4">
    ...
    <ul className="flex flex-wrap gap-4">
      {uploadedFiles.map((file, index) => (
        <li
          key={index}
          className="flex items-center gap-2 text-gray-700 dark:text-[#D0CDEF] hover:underline cursor-pointer"
        >
          <img
